...
const samplePlans = [
  {
    ...
    features: {
      ...
      additionalFeatures: [
        { name: 'Basic Support', description: 'Email and chat support during business hours' },
        { name: 'Standard Installation', description: 'Professional installation included' }
      ]
    },
    availability: {
      isActive: true,
  ...
  },
  {
    ...
    features: {
      ...
      additionalFeatures: [
        { name: 'Priority Support', description: '24/7 phone and chat support' },
        { name: 'Free Router', description: 'High-performance router included' },
        { name: 'Security Suite', description: 'Basic antivirus and firewall protection' }
      ]
    },
    availability: {
      isActive: true,
      regions: ['CA', 'TX', 'NY', 'FL', 'WA', 'CO'],
      startDate: new Date('2024-01-01'),
      endDate: null
    },
    popularityScore: 90
  },
  {
    ...
    features: {
      ...
      additionalFeatures: [
        { name: 'VIP Support', description: 'Dedicated support line with priority response' },
        { name: 'Premium Router', description: 'Latest WiFi 6 router with mesh capability' },
        { name: 'Advanced Security', description: 'Enterprise-grade security and parental controls' },
        { name: 'Static IP', description: 'Static IP address for business needs' }
      ]
    },
    availability: {
      isActive: true,
      regions: ['CA', 'TX', 'NY', 'FL', 'WA', 'CO', 'IL'],
      startDate: new Date('2024-01-01'),
      endDate: null
    },
    popularityScore: 85
  },
  {
    ...
    features: {
      ...
      additionalFeatures: [
        { name: 'Basic Support', description: 'Email support and online resources' },
        { name: 'Standard Modem', description: 'Basic DSL modem included' }
      ]
    },
    availability: {
      isActive: true,
  ...
  },
  {
    ...
    features: {
      ...
      additionalFeatures: [
        { name: 'Standard Support', description: 'Phone and email support' },
        { name: 'Enhanced Modem', description: 'High-speed DSL modem with WiFi' },
        { name: 'Basic Security', description: 'Antivirus software included' }
      ]
    },
    availability: {
      isActive: true,
...
const sampleDiscounts = [
  {
    ...
    conditions: {
      minimumPlanPrice: 25,
      applicablePlans: [],
      applicablePlanTypes: ['Fibernet', 'Broadband Copper'],
...
const generateSampleSubscriptions = (users, plans) => {
  const subscriptions = [];
  const userSubset = users.filter(u => u.role === 'user'); // Only regular users get subscriptions
  
  userSubset.forEach(user => {
    // Each user gets 1-3 subscriptions (some active, some cancelled)
    const numSubscriptions = Math.floor(Math.random() * 3) + 1;
    
    for (let i = 0; i < numSubscriptions; i++) {
      const plan = plans[Math.floor(Math.random() * plans.length)];
      const startDate = new Date();
      startDate.setMonth(startDate.getMonth() - Math.floor(Math.random() * 12)); // Start 0-12 months ago
      
      const endDate = new Date(startDate);
      endDate.setMonth(endDate.getMonth() + 1); // 1 month duration
      
      const isActive = i === 0; // First subscription is active, others might be cancelled
      ...
      subscriptions.push({
        ...
        pricing: {
          ...
          discountAmount: Math.random() > 0.7 ? plan.pricing.monthly * 0.1 : 0,
          finalPrice: plan.pricing.monthly * (Math.random() > 0.7 ? 0.9 : 1),
          currency: 'USD'
        },
        dates: {
          startDate: startDate,
        ...
        usage: {
          currentMonthUsage: isActive ? currentUsage : 0,
          lastMonthUsage: lastMonthUsage,
          averageMonthlyUsage: avgUsage,
...
const generatePaymentHistory = (monthlyPrice, startDate, status) => {
  ...
  for (let i = 0; i <= monthsSinceStart && i < 12; i++) {
    ...
    const paymentStatus = status === 'cancelled' && i === monthsSinceStart ? 'failed' : 
...
const seedDatabase = async () => {
  try {
    ...
    // Update discount createdBy with actual admin user ID
    const adminUser = createdUsers.find(u => u.role === 'admin');
    sampleDiscounts.forEach(discount => {
      discount.createdBy = adminUser.userId;
    });
    ...
    // Generate and create subscriptions
